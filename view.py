...
with bernoulli:
    st.write("""
    **Beta-Bernoulli**: 
    - Treatment rewards for each arm $a$ are distributed as $Bernoulli(\\theta_{a})$ random variables. 
...
with gumbel:
    st.write("""
    **Gamma-Gumbel**: 
    - Treatment rewards for each arm $a$ are distributed as $Gumbel(\mu_{a}, \\beta)$ random variables. 
